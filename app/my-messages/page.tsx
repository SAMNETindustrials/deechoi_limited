...
export default function CustomerMessagesPage() {
  ...
  const loadInquiries = async () => {
    try {
      ...
      console.error('[Error loading customer inquiries]:', err)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSendReply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.data?.id) {
        setMessages((prev) =>
          prev.map((m) => (m.id === tempId ? (data.data as Message) : m))
        )
      }
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] text-[#0A2E1D] font-sans pb-24">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-gray-200 overflow-hidden flex flex-col h-[660px]">
          
          {/* Header */}
          <div className="bg-[#072d1d] text-white p-4 sm:p-5 flex items-center justify-between border-b border-[#EAA823]/30">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-[#EAA823] text-[#072d1d] flex items-center justify-center font-black shadow-md">
                <ShieldCheck className="w-5 h-5" />
              </div>
              <div>
                <h2 className="font-extrabold text-sm sm:text-base leading-tight">De-echoi Kitchen Support</h2>
                <p className="text-[11px] text-emerald-100/80 flex items-center gap-1.5 mt-0.5">
            ...
            {inquiries.length > 1 && (
              <select
                ...
                className="bg-[#041a11] text-white text-xs border border-white/20 rounded-xl px-2.5 py-1.5 outline-none font-medium"
              >
                {inquiries.map((inq) => (
                  <option key={inq.id} value={inq.id}>
                    {inq.subject || 'Inquiry'} ({new Date(inq.last_message_at).toLocaleDateString()})
                  </option>
                ))}
              </select>
            )}
          ...
          {/* Messages Body */}
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-3.5 bg-[#FDFBF7]/60">
            {loading ? (
              ...
              messages.map((msg) => {
                ...
                // Resilient metadata normalization (avoids blank/unclickable circles)
                const rawMeta = typeof msg.metadata === 'string' ? JSON.parse(msg.metadata || '{}') : (msg.metadata || {})
                ...
                // Build rich checkout URL with full item customization specifications
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-1 text-[10px] text-gray-400 mb-1 px-1">
                      <span>{isAdmin ? 'De-echoi Support' : 'You'}</span>
                      <span>&bull;</span>
                    ...
                    >
                      ...
                      {/* Interactive Custom Invoice & Direct Link Payment Card */}
                      {isPayment && (
                        <div className="bg-[#041a11] text-white rounded-2xl p-4 border border-[#EAA823]/50 space-y-3 mt-2 shadow-xl">
                          ...
                          {/* Item Customization Specs Chips */}
                          ...
                          {paymentStatus === 'pending' && (
                            <div className="space-y-2 pt-1">
                              {/* 1. Direct Clickable Link Card to Checkout */}
                              <button
                                type="button"
                              ...
                              {/* 2. Direct In-Chat Transfer Receipt Uploader */}
